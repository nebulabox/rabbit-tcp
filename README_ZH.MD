...
以下列举两种常用用法，详细的文档请移步 [Wiki](https://github.com/nebulabox/rabbit-tcp/wiki)。
...
2. 下载并修改 `docker-compose-server.yml` 文件([链接](https://github.com/nebulabox/rabbit-tcp/raw/master/docker-compose-server.yml))
    ```yaml
    rabbit-server:
      image: ihciah/rabbit
      ports:
...
#### 客户端
1. 安装 [Docker](https://docs.docker.com/install/linux/docker-ce/debian/#install-using-the-convenience-script) 和 [docker-compose](https://docs.docker.com/compose/install/)
2. 下载并修改 `docker-compose-client.yml` 文件([链接](https://github.com/nebulabox/rabbit-tcp/raw/master/docker-compose-client.yml))
    ```yaml
    rabbit-client:
      image: ihciah/rabbit
      ports:
